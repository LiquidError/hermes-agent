...
export const zh: Translations = {
  ...
  boot: {
    ...
    failure: {
      ...
      retry: '重试',
      setUpRemoteGateway: '设置远程网关',
      remoteGatewaySettings: '远程网关设置',
      openLogs: '打开日志',
      setupTitle: '连接到你的 Hermes 后端',
      setupDescription: '此应用本身不运行智能体，而是驱动运行在另一台机器上的 Hermes 仪表盘。请先指向你的后端以开始使用。',
      setupHint: '只需设置一次。该地址是你的仪表盘，例如 http://your-host:9119。',
      genericHint: '请确认仪表盘正在运行且可访问，然后重试——或在网关设置中更新其地址。',
      remoteSignInHint: '打开网关登录窗口以重新建立会话。',
