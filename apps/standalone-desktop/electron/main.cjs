...
async function ensureBackend(profile) {
  ...
  entry.connectionPromise = spawnPoolBackend(key).catch(error => {
...
async function spawnPoolBackend(profile) {
  ...
  // Remote-only client: a named profile must point at its own remote backend
  // (connection.json `profiles[name]`) or inherit the app-wide remote. There
  // is no local child to spawn, so a profile without a reachable remote is a
  // configuration error rather than a reason to bootstrap a local install.
  backendPool.delete(profile)
  throw new Error(
    `Profile "${profile}" has no remote Hermes backend configured. Add a remote URL for it ` +
      'under Settings → Gateway → Remote gateway, or remove the profile.'
  )
}
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    ...
    // Remote-only client: there is no bundled local backend to spawn. If no
    // remote is configured (env override, global settings, or per-profile),
    // surface a clear, actionable error. The renderer treats a rejected
    // connection as "not connected yet" and shows the Remote-gateway setup
    // screen so the user can point at their Hermes dashboard and sign in.
    updateBootProgress({
      phase: 'backend.needs-remote',
      message: 'No remote Hermes backend configured',
      progress: 0,
      running: false,
      error: 'no-remote-configured'
    })
    throw new Error(
      'No remote Hermes backend configured. Open Settings → Gateway → Remote gateway, ' +
        'enter your dashboard URL (e.g. http://<host>:9119) and sign in.'
    )
  })().catch(error => {
